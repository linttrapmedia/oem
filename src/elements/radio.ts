...
const sizeConfig: Record<RadioSize, { size: string; fontSize: string }> = {
...
export const radio = (props: RadioProps) => {
  const { name, value, checked = false, size = 'md', disabled = false, label, onChange } = props;

  const config = sizeConfig[size];

  // Radio input element
  const inputElement = tag.input(
    trait.attr('type', 'radio'),
    trait.attr('name', name),
