import { $test, State, Template, Test, useTextContentTrait } from '@/registry';
...
export const CanApplyInnerHTMLTraitToHtml: Test = async () => {
  ...
  const [tmpl, trait] = Template({ html: useInnerHTMLTrait, text: useTextContentTrait });
  ...
  const e3 = tmpl.div(trait.html(() => tmpl.div(trait.text('one'))));
  const t3 = e3.outerHTML === '<div><div>one</div></div>';
  const e4 = tmpl.div(trait.html(() => [tmpl.div(trait.text('one')), tmpl.div(trait.text('two'))]));
...
export const CanApplyInnerHTMLTraitToSvg: Test = async () => {
  ...
  const e3 = tmpl.circle(trait.html(() => tmpl.circle(trait.text('one'))));
  const t3 = e3.outerHTML === '<circle><circle>one</circle></circle>';
  const e4 = tmpl.circle(
    trait.html(() => [tmpl.circle(trait.text('one')), tmpl.circle(trait.text('two'))]),
  );
