...
(() => {
  ...
  var useStyle = ({
    ...
    return (el, prop, val, condition) => {
      var _a;
      if (mediaType === "print")
        el.dataset.printId = (_a = el.dataset.printId) != null ? _a : (Math.random() * 1e8).toFixed(0);
      const apply = () => {
        if (mediaType === "print") {
          ...
          if (!isInBreakpoint)
            return;
          const _val = String(typeof val === "function" ? val() : val);
          (typeof condition === "function" ? condition() : condition != null ? condition : true) ? el.style[prop] = _val : null;
        }
      };
      if (state)
        state.sub(apply);
      if (event)
        (el != null ? el : eventElement).addEventListener(event, apply);
